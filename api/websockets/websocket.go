	...
	for {
		...
		if err == nil {
			if ws.Verbose && ws.Logger != nil {
				ws.Logger.Info(fmt.Sprintf("Websocket[%s].Dial: connection was successfully established with %s\n", ws.Id, ws.url))
			}
			if ws.OnConnect != nil {
				ws.OnConnect(ws)
			}
			...
			if ws.Verbose && ws.Logger != nil {
				ws.Logger.Error(fmt.Sprintf("Websocket[%s].Dial: can't connect to %s, will try again in %v\n", ws.Id, ws.url, nextInterval))
			}
